...
class TestHarmonyAnalyzer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.dissonant_audio = (
            0.5 * np.sin(2 * np.pi * 261.63 * t) +  # C
            0.5 * np.sin(2 * np.pi * 277.18 * t)    # C# (minor second)
        )
    ...
    def test_initialization(self):
        ...
        custom_analyzer = HarmonyAnalyzer(
            hop_length=256, 
            consonance_ratings=custom_consonance
        )
    ...
    def test_analyze_harmonic_rhythm(self):
        ...
        self.assertGreater(
            harmonic_rhythm['chord_change_rate'],
            static_rhythm['chord_change_rate']
        )
    ...
    def test_consonance_ratings_effect(self):
        ...
        self.assertGreater(
            high_consonance['consonance_score'],
            low_consonance['consonance_score']
        )
    ...
    def test_interval_consonance_calculation(self):
        ...
        fifth_audio = (
            np.sin(2 * np.pi * 440 * np.linspace(0, 2, self.sr * 2)) +
            np.sin(2 * np.pi * 660 * np.linspace(0, 2, self.sr * 2))  # 3:2 ratio
        )
        
        # Create tritone (dissonant)
        tritone_audio = (
            np.sin(2 * np.pi * 440 * np.linspace(0, 2, self.sr * 2)) +
            np.sin(2 * np.pi * 622 * np.linspace(0, 2, self.sr * 2))  # Tritone
        )
