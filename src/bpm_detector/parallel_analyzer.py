...
from .auto_parallel import AutoParallelConfig, SystemMonitor, PerformanceProfiler, ParallelConfig
from .progress_manager import ProgressManager, ProgressCallback, TaskStatus
...
class SmartParallelAudioAnalyzer(AudioAnalyzer):
    ...
    def _configure_auto_parallel(self):
        ...
        if self._parallel_config.enable_parallel:
            ...
            print(f"   Memory limit: {self._parallel_config.memory_limit_mb}MB ({memory_limit_gb:.1f}GB, {memory_percentage:.0f}% of {total_memory_gb:.1f}GB physical)")
        ...
        **kwargs
        ...
        if isinstance(path, (list, tuple)):
            return self._analyze_multiple_files(
                path, comprehensive, progress_callback, progress_display, detailed_progress, **kwargs
            )
        else:
    ...
    def _parallel_comprehensive_analysis(
        ...
        **kwargs
        ...
        basic_info = self._analyze_basic_info_with_progress(y, sr, path, basic_callback, **kwargs)
        ...
        with ThreadPoolExecutor(max_workers=effective_workers) as executor:
            futures = {
                'chord_progression': executor.submit(
                    self._analyze_with_progress, 'chord_progression',
                    progress_manager, self.chord_analyzer.analyze, y, sr, key, float(basic_info.get('bpm', 130.0))
                ),
                'structure': executor.submit(
                    self._analyze_with_progress, 'structure',
                    progress_manager, self._analyze_structure_with_progress, y, sr, float(basic_info.get('bpm', 130.0))
                ),
                'rhythm': executor.submit(
                    self._analyze_with_progress, 'rhythm',
                    progress_manager, self.rhythm_analyzer.analyze, y, sr
                ),
                'timbre': executor.submit(
                    self._analyze_with_progress, 'timbre',
                    progress_manager, self.timbre_analyzer.analyze, y, sr
                ),
                'melody_harmony': executor.submit(
                    self._analyze_with_progress, 'melody_harmony',
                    progress_manager, self.melody_harmony_analyzer.analyze, y, sr
                ),
                'dynamics': executor.submit(
                    self._analyze_with_progress, 'dynamics',
                    progress_manager, self.dynamics_analyzer.analyze, y, sr
                )
            }
    ...
    def _analyze_structure_with_progress(self, y, sr, callback, bpm=130.0):
        ...
        boundaries = self.structure_analyzer.detect_boundaries(similarity_matrix, sr, bpm=bpm)
            ...
            if len(segment) > sr:  # Only analyze segments longer than 1 second
                try:
                    ...
                    if segment_chords:
                        ...
                        section_progression = ' → '.join(unique_chords) if unique_chords else 'Unknown'
                    else:
