...
class FeatureAnalyzer:
    ...
    def analyze_segment_characteristics_enhanced(self, segment: np.ndarray, sr: int,
    ...
    def analyze_segment_characteristics(self, segment: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'tempo': float(tempo) if np.isscalar(tempo) else float(tempo[0]) if len(tempo) > 0 else 120.0,
    ...
    def _detect_spoken_word(self, segment: np.ndarray, sr: int, energy: float, complexity: float) -> bool:
        ...
        try:
            ...
            is_spoken = (
                energy < energy_threshold * 2 and  # Lower energy
                1000 < spectral_centroid < 3000 and  # Speech frequency range
                zero_crossing_rate > 0.1 and  # Higher ZCR
                complexity < 0.5  # Lower spectral complexity
            )
    ...
    def _detect_vocal_presence(self, segment: np.ndarray, sr: int) -> bool:
        ...
        try:
            ...
            has_vocals = (
                harmonic_ratio > 0.3 and  # Sufficient harmonic content
                1000 < spectral_centroid < 4000  # Vocal frequency range
            )
