...
class StructureAnalyzer:
    ...
    def analyze(self, y: np.ndarray, sr: int, bpm: float = 130.0) -> Dict[str, Any]:
        ...
        energy_scale_info = self.section_processor.calculate_energy_scale(y)
    ...
    def detect_boundaries(self, similarity_matrix: np.ndarray,
                         sr: int, min_segment_length: float = 12.0, bpm: float = 130.0) -> List[int]:
    ...
    def classify_sections(self, y: np.ndarray, sr: int,
                         boundaries: List[int], bpm: float = 130.0) -> List[Dict[str, Any]]:
