...
class SectionProcessor:
    ...
    def post_process_sections(self, raw: List[Dict[str, Any]],
                             total_duration: float = None,
                             merge_threshold: float = None,
                             bpm: float = 130.0,
                             y: np.ndarray = None,
                             sr: int = None) -> List[Dict[str, Any]]:
        ...
        for i, seg in enumerate(merged):
            ...
            if (len(enhanced) > 0 and enhanced[-1]['type'] == 'chorus' and
                seg['type'] == 'instrumental' and seg.get('vocal_ratio') is not None):
                should_absorb = False
            ...
            if should_absorb:
                ...
                if seg['duration'] > min_dur * 0.3:  # If absorbed segment is substantial
                    ...
                    if seg['duration'] > enhanced[-1]['duration'] * 0.5:
                        ...
                        merged_type = self._smart_merge_types(prev_type, curr_type,
                                                            enhanced[-1]['duration'], seg['duration'])
        ...
        for sec in enhanced:
            if sec["type"] in ["verse", "pre_chorus"] and sec["duration"] > split_sec:
                ...
                first = sec.copy(); last = sec.copy()
                first["end_time"] = mid;  first["duration"] = mid - first["start_time"]
                # 先半分は Verse とする
                first["type"] = "verse"
                first["ascii_label"] = self.JP_ASCII_LABELS["verse"]
                # 後半は Pre-Chorus とする
                last["start_time"] = mid; last["duration"] = sec["end_time"] - mid
    ...
    def _cleanup_ending_instrumentals(self, sections: List[Dict[str, Any]], bpm: float = 130.0) -> List[Dict[str, Any]]:
        ...
        if processed:
            ...
            # Convert ending instrumental or short verse to outro (≤8 bars)
            if (last['type'] in ['instrumental', 'verse']
                and last_dur <= eight_bar_sec):
                last['type'] = 'outro'
    ...
    def refine_section_labels_with_spectral_analysis(self, y: np.ndarray, sr: int,
    ...
    def _classify_instrumental_subtype(self, section: Dict[str, Any],
                                     spectral_features: Dict[str, Any] = None) -> str:
    ...
    def _break_consecutive_chorus_chains(self, sections: List[Dict[str, Any]],
