...
class ProgressManager:
    ...
    def get_running_tasks(self) -> List[str]:
        ...
        with self._lock:
            return [
                task.name for task in self._tasks.values() 
    ...
    def update(self, progress_manager: ProgressManager):
        ...
        try:
            ...
            with progress_manager._lock:
                overall_progress = sum(task.progress for task in progress_manager._tasks.values()) / len(progress_manager._tasks) if progress_manager._tasks else 0
                running_tasks = [task.name for task in progress_manager._tasks.values() if task.status == TaskStatus.RUNNING]
...
class DetailedProgressDisplay(ProgressDisplay):
    ...
    def update(self, progress_manager: ProgressManager):
        ...
        try:
            ...
            # Calculate overall progress
            overall_progress = sum(task.progress for task in task_details.values()) / len(task_details) if task_details else 0
